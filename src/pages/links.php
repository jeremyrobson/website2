<h1>Here are some links</h1>

<ul>
    <li>
        <a href="https://www.soundcloud.com/jeremyrobson">My Soundcloud</a>
    </li>
    <li>
        <a href="https://www.github.com/jeremyrobson">My Github</a>
    </li>
    <li>
        <a href="http://ocremix.org/artist/4559/jeremy-robson/remixes">My OCReMix</a>
    </li>
    <li>
        <a href="https://www.youtube.com/channel/UCfBk-TS4bOK2t0JFcFNufHw">My Youtube</a>
    </li>
    <li>
        <a href="https://codepen.io/jeremyrobson/">My Codepen</a>
    </li>
    <li>
        <a href="http://soundclick.jeremyrobson.com">My SoundClick</a>
    </li>
    <li>
        <a href="instagram">My Instagram</a>
    </li>
</ul>
